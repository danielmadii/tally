...
export default function HomeScreen({ firstName }: { firstName: string }) {
  ...
  return (
    <div className="mx-auto max-w-md px-4 pt-6">
      ...
      <p className="text-sm text-slate-500">{t("daySoFar")}</p>

      <div className="mt-5 grid grid-cols-3 gap-3">
        <Stat label={t("today")} value={isLoading ? "…" : fmtMoney(stats?.todayRevenue ?? 0)} />
        <Stat label={t("units")} value={isLoading ? "…" : fmtInt(stats?.todayUnits ?? 0)} />
        <Stat label={t("shopRank")} value={isLoading ? "…" : rankLabel} />
      </div>
      ...
      <div className="mt-8 flex flex-col items-center card p-6">
        <ProgressRing
          ...
          sublabel={t("ofMonthlyTarget")}
        />
        <p className="mt-4 text-sm text-slate-500">
          {stats?.monthTarget
            ? `${fmtMoney(stats.monthRevenue)} / ${fmtMoney(stats.monthTarget)} — ${t("thisMonth")}`
            : t("noTargetSet")}
        </p>
      </div>
